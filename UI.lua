...
local MainAssetFolder = Debug and ReplicatedStorage.RTX CPM
...
local function InitScreen()
	local ScreenAsset = GetAsset("Screen/RTX CPM")
	if not Debug then sethiddenproperty(ScreenAsset,"OnTopOfCoreBlur",true) end
	ScreenAsset.Name = "RTX CPM " .. game:GetService("HttpService"):GenerateGUID(false)
...
local RTX CPM = InitScreen()
function RTX CPM:Window(Window)
	...
	local WindowAsset = InitWindow(RTX CPM.ScreenAsset,Window)
	function Window:Tab(Tab)
		Tab = GetType(Tab,{},"table")
		Tab.Name = GetType(Tab.Name,"Tab","string")
		local ChooseTab = InitTab(RTX CPM.ScreenAsset,WindowAsset,Window,Tab)
		...
		function Tab:Button(Button)
			...
			InitButton(ChooseTab(Button.Side),RTX CPM.ScreenAsset,Window,Button)
			return Button
		end
		function Tab:Toggle(Toggle)
			...
			InitToggle(ChooseTab(Toggle.Side),RTX CPM.ScreenAsset,Window,Toggle)
			return Toggle
		end
		function Tab:Slider(Slider)
			...
			InitSlider(ChooseTab(Slider.Side),RTX CPM.ScreenAsset,Window,Slider)
			return Slider
		end
		function Tab:Textbox(Textbox)
			...
			InitTextbox(ChooseTab(Textbox.Side),RTX CPM.ScreenAsset,Window,Textbox)
			return Textbox
		end
		function Tab:Keybind(Keybind)
			...
			InitKeybind(ChooseTab(Keybind.Side),RTX CPM.ScreenAsset,Window,Keybind)
			return Keybind
		end
		function Tab:Dropdown(Dropdown)
			...
			InitDropdown(ChooseTab(Dropdown.Side),RTX CPM.ScreenAsset,Window,Dropdown)
			return Dropdown
		end
		function Tab:Colorpicker(Colorpicker)
			...
			InitColorpicker(ChooseTab(Colorpicker.Side),RTX CPM.ScreenAsset,Window,Colorpicker)
			return Colorpicker
		end
		function Tab:Section(Section)
			...
			function Section:Button(Button)
				...
				Button.Name = GetType(Button.Name,"Button","string")
				Button.Callback = GetType(Button.Callback,function() end,"function")
				InitButton(SectionContainer,RTX CPM.ScreenAsset,Window,Button)
				return Button
			end
			function Section:Toggle(Toggle)
				...
				InitToggle(SectionContainer,RTX CPM.ScreenAsset,Window,Toggle)
				return Toggle
			end
			function Section:Slider(Slider)
				...
				InitSlider(SectionContainer,RTX CPM.ScreenAsset,Window,Slider)
				return Slider
			end
			function Section:Textbox(Textbox)
				...
				InitTextbox(SectionContainer,RTX CPM.ScreenAsset,Window,Textbox)
				return Textbox
			end
			function Section:Keybind(Keybind)
				...
				InitKeybind(SectionContainer,RTX CPM.ScreenAsset,Window,Keybind)
				return Keybind
			end
			function Section:Dropdown(Dropdown)
				...
				InitDropdown(SectionContainer,RTX CPM.ScreenAsset,Window,Dropdown)
				return Dropdown
			end
			function Section:Colorpicker(Colorpicker)
				...
				InitColorpicker(SectionContainer,RTX CPM.ScreenAsset,Window,Colorpicker)
				return Colorpicker
			end
			return Section
		end
...
function RTX CPM:TableToColor(Table)
	if type(Table) ~= "table" then return Table end
	return Color3.fromHSV(Table[1],Table[2],Table[3])
end

function RTX CPM:Notification(Notification)
	...
	NotificationAsset.Parent = RTX CPM.ScreenAsset.NDHandle
...
function RTX CPM:Notification2(Notification)
	Notification = GetType(Notification,{},"table")
	Notification.Title = GetType(Notification.Title,"Title","string")
	...
	NotificationAsset.Parent = RTX CPM.ScreenAsset.NLHandle
